...
*/
void host_read_header(msg_header_t *hdr) {
    ...
    host_read_bytes(&hdr->len, sizeof(hdr->len));
}
 ...
 *  @return 0 on success. A negative value on error.
*/
int host_write_bytes(const void *buf, uint16_t len, bool should_ack) {
    for (int i = 0; i < len; i++) {
        if (i % 256 == 0 && i != 0) {  // Expect an ACK after sending every 256 bytes
            if (should_ack && host_read_ack() < 0) {
                return -1;
            }
        }
...
int host_write_hex(msg_type_t type, const void *buf, size_t len) {
    ...
    host_write_bytes(&hdr, MSG_HEADER_SIZE, false /* should_ack */);
    if (type != DEBUG_MSG && host_read_ack() < 0) {
...
int host_write_packet(msg_type_t type, const void *buf, uint16_t len) {
    ...
    result = host_write_bytes(&hdr, MSG_HEADER_SIZE, false);
    if (type == ACK_MSG) {
        return result;
    }
    ...
    if (len > 0) {
        result = host_write_bytes(buf, len, type != DEBUG_MSG);
